---
name: housekeeping
description: Comprehensive repository and memory housekeeping — audits .env and secrets, reconciles ignore layers, synchronizes dual-layer developer memory, cleans workspace debris, and validates build health. Use when performing hygiene sweeps, auditing secrets, or synchronizing developer database vectors.
---

# Repository & Memory Housekeeping

> **Persona: The Custodian**  
...
* **Secrets Isolation**: Verify that `.env` keys remain strictly ignored by Git while `.env.example` contains sanitized placeholders.
* **Dual-Layer Memory Parity**: Synchronize living vector memory (Pinecone) and cold storage (Supabase) to reflect the current architectural state.
* **Workspace Hygiene**: Keep the repository root free of transient files and diagnostics; all scratch scripts belong in `tmp/**`.
* **Zero Technical Debt Lingering**: Sweep `#TODO-AI` tags into `tasks/PRESENT.md` and prune dead code per P4 Zero Backwards Compatibility.

---

## 2.0 The 5-Stage Housekeeping Protocol

Triggered during maintenance routines or explicitly via `/housekeeping`.
...
3. **Token Synchronization (When Applicable)**:
   * Run token sync scripts (e.g. `npm run sync:css`) if design tokens were altered.
...
1. **Mission Board Update (`tasks/PRESENT.md`)**:
   ...
   * Record an entry in `## 📜 Past` detailing the housekeeping maintenance actions, active skills (`housekeeping`), and status `✅ Completed`.
...
3. **Housekeeping Certification Briefing**:
   * Conclude with a categorized summary table:
     ...
     | **Developer Memory** | 🟢 Synced | Living vectors upserted; cold storage verified. |
     | **Workspace Hygiene** | 🟢 Clean | Root directory free of clutter; `tmp/` pruned; debt swept. |
     | **Verification Gate** | 🟢 Green | Hook contracts passing; zero linter errors; test suites green. |
...
* **Memory Neglect**: Leaving vector memory stale after significant structural refactors.
