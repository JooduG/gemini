...
9. **Apply `.understandignore` exclusions** (same semantics as `/understand` Step 2.5 in `agents/project-scanner.md`).
   ...
   3. Resolve `$PLUGIN_ROOT`:
      - Use `$ANTIGRAVITY_PLUGIN_ROOT` if set (Antigravity IDE's hook context sets this).
      ...
      - If neither resolves: report "Cannot locate plugin install at `$ANTIGRAVITY_PLUGIN_ROOT` or `$HOME/.understand-anything-plugin`; auto-update aborted. Run `/understand` to re-baseline." and **STOP**. Do **not** silently skip — silent skip reproduces issue #153.
...
1. Read `.understand-anything/intermediate/change-analysis.json`.
